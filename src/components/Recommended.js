		...
		<div className={classes.root} >
			...
			<Grid container spacing={2} className={classes.containerGrid} >
				<Grid item xs={12} sm={6} md={3} lg={3} key={first.id} style={{ height: '100%' }}>
					<DeviceCard name={first.phoneName} price={first.prices.twelvePrice} desc={first.description} image={first.imageIds.filter(image => image.imageName.includes('thumb_1_main'))} id={first.id}/>
				</Grid>
				<Grid item xs={12} sm={6} md={3} lg={3} key={second.id} style={{ height: '100%' }}>
					<DeviceCard name={second.phoneName} price={second.prices.twelvePrice} desc={second.description} image={second.imageIds.filter(image => image.imageName.includes('thumb_1_main'))} id={second.id}/>
				</Grid>
				<Grid item xs={12} sm={6} md={3} lg={3} key={third.id} style={{ height: '100%' }}>
					<DeviceCard name={third.phoneName} price={third.prices.twelvePrice} desc={third.description} image={third.imageIds.filter(image => image.imageName.includes('thumb_1_main'))} id={third.id}/>
				</Grid>
				<Grid item xs={12} sm={6} md={3} lg={3} key={fourth.id} style={{ height: '100%' }}>
					<DeviceCard name={fourth.phoneName} price={fourth.prices.twelvePrice} desc={fourth.description} image={fourth.imageIds.filter(image => image.imageName.includes('thumb_1_main'))} id={fourth.id}/>
				</Grid>
			</Grid>
		</div>
	)
